...
$prefix="festember_";
	...
	function submitCaptcha(){
		if($_POST['captcha'])
			...
			$query = "SELECT `value` FROM `". $prefix ."global` WHERE `attribute`='recaptcha_private'";
	...
	function submitRegistrationFormMember($moduleCompId, $elementId, $userId, $postVarName, $elementName, $elementSize, $elementTypeOptions, $elementMoreThan, $elementLessThan, $elementCheckInt, $elementIsRequired) {
		...
		if($submitData != "") {
		  if(!is_numeric($submitData)) {
		    ...
		    $query = 'SELECT `user_id` FROM `'.$prefix."users` WHERE `user_email` = '".$submitData."'";
		    $result = mysql_query($query);
		  ...
		  $query = 'SELECT * FROM `'.$prefix."users` WHERE `user_id` = '".$submitData."'";
		  $result = mysql_query($query);
		  if(!mysql_num_rows($result)) {
