...
function upload($moduleComponentId, $moduleName, $userId, $uploadFormName, $maxFileSizeInBytes=false, $uploadableFileTypesArray = false) {
	...
	global $sourceFolder;global $prefix;
	global $uploadFolder;
	$uploadDir = "uploads";
	...
	if (!file_exists('uploads')) {
		displaywarning("The folder uploads does not exist. Trying to create it");
		mkdir('uploads', 0755);
		if (!file_exists("uploads")) {
			displayerror("Creation of directory failed");
			return false;
...
function saveUploadedFile($moduleComponentId,$moduleName, $userId, $uploadFileName, $tempFileName, $uploadFileType, $uploadDir) {
	global $prefix;
	$query = 'SELECT MAX(`upload_fileid`) FROM `' . $prefix . 'uploads`';
	...
	if(strpbrk($uploadFileName, '%#&')) {
		...
		$duplicateCheckQuery = "SELECT * FROM `" . $prefix . "uploads` " .
		...
		$query = 'INSERT INTO `' . $prefix . 'uploads` ' .
...
function getUploadedFiles($moduleComponentId, $moduleName) {
	$query = "SELECT `upload_filename`, `upload_filetype`, `upload_time`, `user_id` FROM `" . $prefix . "uploads` WHERE `page_modulecomponentid` ='" . $moduleComponentId . "' AND `page_module` = '" . $moduleName . "'";
...
function fileCopy($sourcePage_modulecomponentid,$sourcePage_module,$sourceFile_name,
	...
	$query = "SELECT * FROM `" . $prefix . "uploads` WHERE `page_modulecomponentid` ='" . $sourcePage_modulecomponentid . "' AND `upload_filename` ='" . mysql_escape_string($sourceFile_name) . "'";
	$result = mysql_query($query);
	...
	$query1= "SELECT MAX(upload_fileid) as MAX FROM `" . $prefix . "uploads` ";
	...
	if(!copy($tmp_name, $uploadDir . "/" . $destinationPage_module . "/" . $finalname))
	   return false;
	$query2 = "INSERT INTO `" . $prefix . "uploads` (`page_modulecomponentid` ,`page_module` , `upload_fileid` , `upload_filename` ," .
...
function getFileName($moduleComponentId, $page_module, $upload_fileid) {
	$query = " SELECT * FROM `" . $prefix . "uploads` WHERE `page_modulecomponentid` ='$moduleComponentId' AND `page_module` ='$page_module' AND `upload_fileid` ='$upload_fileid'";
	$result = mysql_query($query);
...
function deleteFile( $moduleComponentId, $page_module, $upload_filename) {
	...
	$query = "SELECT * FROM `" . $prefix . "uploads` WHERE `page_modulecomponentid` ='$moduleComponentId' AND `page_module` = '$page_module' AND `upload_filename`= '" . mysql_escape_string($upload_filename) . "'";
	...
	$query = "DELETE FROM `" . $prefix . "uploads` WHERE `upload_fileid`='$upload_fileid'";
